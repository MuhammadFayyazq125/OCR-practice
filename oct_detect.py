...
# This function converts a pixmap buffer representing a screenshot taken using the PyMuPDF 
# library into a NumPy array.
...
def convert_img2bin(img):
    ...
    # Converting image to binary by Thresholding in order to show a clear separation between white
    #  and blacl pixels.
    output_img = threshold(output_img)
    return output_img

# We have defined functions for many preprocessing tasks, including converting 
# images to grayscale, flipping pixel values, separating white and black pixels, and much more.
...
# The display_img() function displays on-screen an image in a window having a title set to the
# title parameter and maintains this window open until the user presses a key on the keyboard.
...
# We will be using this function for searching specific text 
# within the grabbed content of an image. It returns a generator of the found matches.
...
# save_page_content() function appends the grabbed content
#  of an image line by line after scanning it to the pdfContent pandas dataframe.
...
# Next, let's write a 
# function that calculates the confidence score of the text grabbed from the scanned image:
